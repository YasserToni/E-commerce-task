...
function CardsDiscount({ discountPercentage }) {
  ...
  return (
    <div className="w-64 sm:w-[168px] h-96 sm:h-fit p-3 rounded-xl product-card-shadow relative mb-3 ">
      {discountPercentage ? (
        <span className="flex items-center font-medium text-sm sm:text-[10px] rounded-xl  py-1 sm:py-0 px-3 sm:px-1  bg-[#FF2828] font-fm text-[#FFFFFF]  absolute top-10 sm:top-6 right-4">
        ...
        <SheetTrigger>
          <div className="text-[#FFFFFF] sm:w-6 sm:h-6 w-[36px] h-[34px]">
            <img
              ...
              className="absolute left-3 sm:left-2  bottom-28 sm:bottom-16 hover:cursor-pointer py-1 px-1.5 bg-[#3074F0] rounded-md sm:w-6 sm:h-6 w-[36px] h-[34px]"
            />
          </div>
        </SheetTrigger>
      ...
      <Link to="/product">
        <img
          className="max-w-60 sm:max-w-[108px] max-h-64 sm:max-h-[144px] pb-1  mx-auto cursor-pointer"
          src={productImage}
          alt="product image"
        />
      </Link>
      ...
      <div>
        <Link to="/product">
          <h3 className="font-normal text-lg sm:text-xs sm:leading-4 font-fr text-[#454545] line-clamp cursor-pointer">
            سامسونج الترا جالاكسي. شريحتان اتصال بلون بنفسجي.
          </h3>
        </Link>
        <div className="flex items-center justify-between">
          <div>
            <span className="font-bold text-xl sm:text-xs sm:leading-5 sm:text-[#454545] text-[#E02954]">
              6500ج
            </span>
